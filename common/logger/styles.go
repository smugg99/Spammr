...
func GetDefaultLoggerStyle() *log.Styles {
	...
	styles.Levels[log.FatalLevel] = lipgloss.NewStyle().
		...
		Bold(true).
		Blink(true)
	styles.Levels[log.Level(ProgressLevel)] = lipgloss.NewStyle().
		SetString("[PROG]").
		Padding(0, 7, 0, 0).
		Foreground(lipgloss.Color("#FFFF00")).
		Bold(true)

	styles.Levels[log.Level(ProgressErrorLevel)] = lipgloss.NewStyle().
		SetString(fmt.Sprintf("%s %s", styles.Levels[log.Level(ProgressLevel)].Copy().Padding(0, 0, 0, 0), lipgloss.NewStyle().
			SetString("[ERRO]").
			Padding(0).
			Foreground(lipgloss.Color("#FF0000")).
			Bold(true)))

	styles.Levels[log.Level(ProgressDebugLevel)] = lipgloss.NewStyle().
		SetString(fmt.Sprintf("%s %s", styles.Levels[log.Level(ProgressLevel)].Copy().Padding(0, 0, 0, 0), lipgloss.NewStyle().
			SetString("[DEBU]").
			Padding(0).
			Foreground(lipgloss.Color("#1E90FF")).
